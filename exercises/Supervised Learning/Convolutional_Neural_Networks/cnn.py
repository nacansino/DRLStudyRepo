...
# Annotated by Jay Cansino for personal study
...
#sample using one dataset

from PIL import Image
from keras.preprocessing.image import load_img, img_to_array

#you can import the whole image class from keras.preprocessing 

def l_img_prepr(img,size):
    x = img_to_array(img.resize(size))
    #expand dimensions (because the classifier expects 4dimensions, 
    # with the first dimension being the batch number)
    x = x.reshape((1,) + x.shape) 
    return x
    
img = [Image.open('dataset/single_prediction/cat_or_dog_1.jpg'),
       Image.open('dataset/single_prediction/cat_or_dog_2.jpg')
       ]

#apply resizing to array of images
x1 = l_img_prepr(img[0],(64,64))
x2 = l_img_prepr(img[1],(64,64))
